...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################    
    da = dnext_h * ( 1 - np.tanh(np.dot(x,Wx)+np.dot(prev_h,Wh)+b)**2 )
    dx = np.dot(da,Wx.T)
    dWx = np.dot(x.T,da)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        xt = x[:,t,:]
        next_h, cache[t] = rnn_step_forward(xt,prev_h,Wx,Wh,b)
        h[:,t,:] = next_h
        prev_h = next_h
    ##############################################################################
    #                               END OF YOUR CODE                             #
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dx[:,t,:], dprev_h, dWx_t, dWh_t,db_t = rnn_step_backward(dh[:,t,:]+dnext_h,cache[t])
        dnext_h = dprev_h
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dai = dc * g * sigmoid(ai) * (1-sigmoid(ai))     
    daf = dc * prev_c * sigmoid(af) * (1-sigmoid(af))    
    dao = dnext_h * np.tanh(next_c) * sigmoid(ao) * (1-sigmoid(ao))    
    dag = dc * i * (1-np.tanh(ag)**2)
